...
15. **Approved Phase 0 semantic contract governs implementation planning.** [`EIP_Taxonomy_v2.md`](EIP_Taxonomy_v2.md:1), version `2.0.0`, defines the required entity boundaries, evidence anchors, and classification processing order: `ContextType → Topic → Category → enrichment`. It authorises separate implementation planning only; it does not alter deployed Azure extraction behaviour or authorise runtime configuration changes.
16. **Cloudflare Topic Memory is the migration target under dual-write.** The existing Azure pipeline remains authoritative during coexistence while dual-writing idempotent evidence and extraction candidates to a Cloudflare Worker/API-backed Topic Memory. Cloudflare D1 will store canonical relational memory and append-only events; Cloudflare R2 will hold permitted immutable source-object captures and extraction snapshots. Cutover requires an approved pilot duration, full source reconciliation, no unresolved high-severity divergence, verified access/audit/retention/redaction controls, tested replay/recovery, and a named owner’s recorded approval. See [`EIP_Taxonomy_v2.md`](EIP_Taxonomy_v2.md:184). This Phase 0 approval does not authorise dual-write implementation or cutover.
